...
#show: template.with(
  title: (
    ...
    en: (
      "22. WebNavix: Continual Generalist Web Navigation Agent",
      "with Domain-wise Mixture-of-Experts",
    ),
  ),
  authors: (
    (ja: "塩畑 晴人", en: "Haruto SHIOHATA"),
    (ja: "池田 耕", en: "Koh IKEDA"),
  ),
  abstract: [
    In this paper, we introduce a domain-wise model merging approach based on the Mixture-of-Experts which continuously integrates individually fine-tuned LLMs as domain experts, named WebNavix, for the first time in the field of Web Navigation. Experimental results demonstrate that WebNavix achieves state-of-the-art performance, surpassing the previous best score by 7.48% in evaluations conducted with WebLINX. Despite this success, WebNavix faces several challenges that must be addressed for practical applications, including compressing the inflated model size during training and extending its applicability to vision-language models.
  ],
  keywords: (
...
#(i)WebNavixは、Mixture-of-Experts（MoE）@jiang2024mixtral に基づくドメイン別モデルマージ手法であるBranch-Train-MiX#(es)@sukhbaatar2024branchtrainmixmixingexpertllms#(es)をWeb Navigationのために拡張する。WebNavixは、Task Adapt、Branch、Domain Adapt、MiXの4つのステージから構成される。
...
$cal(M)_i$#(es)に対する適切な入力表現#(es)$x_i$#(es)#footnote(webnavix-footnotes.appropriate-input-representation)#(es)が与えられたとき、$cal(M)$#(es)の#(es)$L$#(es)層目までの各隠れ状態#(es)$h_i^l$#(es)は次のように計算される。
...
- $cal(D)$：WebLINX
- $cal(M)$：Sheared-LLaMA-2.7B @xia2024shearedllamaacceleratinglanguage
...
#(i)主な結果を@main-results\に示す。本実験におけるWebNavixは量子化された推論であるにもかかわらず、総合スコアの従来SoTAを7.48%更新した。併せて、IoUは31.26%、F1は6.59%向上した一方、IMでは2.16%の低下が見られた。

== ルーティング解析

#(i)WebNavixの性能をより深く調査するために、実験で構築したWebNavixモデルのルーティング解析を行う。この解析は、WebNavixモデルの各層のルーティング結果をヒートマップとして可視化することで行う。理想的には、各層で入力テキストのドメインと割当てられたELMのドメインが一致し、対角線上にヒートマップが現れることが期待されるが、@routing-analysis\に示すように、ヒートマップは水平な帯状として現れた。このことは、WebNavixモデルのゲーティングネットワークは、入力テキストのドメインに一切影響されず、層に影響されてルーティングを決定していることを意味する。この原因として、Task Adapt時のepoch数が多く、その時点でデータセットを十分に学習してしまい、Domain Adapt時の学習で各ELMの能力に十分な差が生じなかったことが考えられる。

= 課題と展望

#(i)Web Navigationのためのモデルは、システムの機密性や可用性の観点から、ユーザーのマシン上で動作できるように十分に軽量であることが望まれる。しかし、本実験で構築したWebNavixモデルのサイズは14.6Bであり、これは量子化などによりモデルサイズを圧縮しなければ、一般的なPCやスマートフォン上で動作させることはできない。
...
#place(
  ...
  [
    ...
    #tbl(
      ...
      caption: [The main results based on WebLINX. Due to budget constraints, WebNavix reports inference results obtained through 4-bit quantization.],
    ) <main-results>
  ],
)
...
#place(
  ...
  [
    ...
    #img(
      ...
      caption: [The routing analysis results. The top left shows the routing results of the 1st layer, the top right shows the 8th layer, the bottom left shows the 16th layer, and the bottom right shows the 32nd layer.],
    ) <routing-analysis>
  ],
)
