...
def main(cfg) -> None:
    ...
    with Path.open(model_save_dir.joinpath(f"{cfg.train.split}/input_records.json"), "r") as f:
        train_input_records = json.load(f)
    with Path.open(model_save_dir.joinpath(f"{cfg.eval.split}/input_records.json"), "r") as f:
...
def train(  # noqa: PLR0913
    ...
    peft_config = LoraConfig(
        ...
        task_type="CAUSAL_LM",
        target_modules=cfg.train.qlora.target_modules,
    )
    ...
    trainer = SFTTrainer(
        ...
        # NOTE: max_seq_length and dataset_text_field are no longer supported in v1.0.0.
